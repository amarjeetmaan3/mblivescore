...
def fetch_match_smart(match_url):
    ...
    try:
        ...
        if full_sc and isinstance(full_sc, list):
            for idx, inn in enumerate(full_sc):
                ...
                # 1. NEW: All Partnerships (Past)
                if "partnershipsData" in inn:
                    for key, p in inn["partnershipsData"].items():
                        past_parts.append({
                            "wktNo": p.get("wicketNum", 0), "bat1Name": p.get("bat1Name", ""), "bat1Runs": p.get("bat1Runs", 0),
                            "bat2Name": p.get("bat2Name", ""), "bat2Runs": p.get("bat2Runs", 0), "totalRuns": p.get("totalRuns", 0), "totalBalls": p.get("totalBalls", 0)
                        })
    ...
    data = {
        ...
        # Current Partnership & Graphs (Recent Balls)
